...
class function TVersionInfo.GetAppVersion: string;
resourcestring
  rsVer = '%s.%s';
var
...
begin
  ...
  try
    Result := Format(rsVer, [VersionInfo.FileVersion, VersionInfo.FileBuild]);
  finally
    FreeAndNil(VersionInfo);
  end;
end;

function TVersionInfo.GetArchitecture: string;
begin
  Result := '';
