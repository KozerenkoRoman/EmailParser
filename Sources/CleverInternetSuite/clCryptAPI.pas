...
type
  TclCryptSetProvParam = function (hProv   :HCRYPTPROV;
                           dwParam :DWORD;
                           pbData  :PBYTE;
                           dwFlags :DWORD) :BOOL;stdcall;
...
type
  TclCryptMsgOpenToDecode = function (dwMsgEncodingType :DWORD;
                              dwFlags :DWORD;
                              dwMsgType :DWORD;
                              hCryptProv :HCRYPTPROV;
                              pRecipientInfo :PCERT_INFO;   //OPTIONAL
...
type
  TclCryptMsgClose = function (hCryptMsg :HCRYPTMSG):BOOL ; stdcall;
...
type
  TclCryptMsgUpdate = function (hCryptMsg :HCRYPTMSG;
                  const pbData :PBYTE;
                        cbData :DWORD;
                        fFinal :BOOL):BOOL ; stdcall;
...
type
  TclCryptMsgControl = function (hCryptMsg :HCRYPTMSG;
                         dwFlags :DWORD;
                         dwCtrlType :DWORD;
                         pvCtrlPara :PVOID):BOOL ; stdcall;
...
type
  TclCryptMsgGetParam = function (hCryptMsg :HCRYPTMSG;
                          dwParamType :DWORD;
                          dwIndex :DWORD;
                          pvData :PVOID;
                          pcbData :PDWORD):BOOL ; stdcall;
...
type
  TclCertGetPublicKeyLength = function (dwCertEncodingType :DWORD;
                                pPublicKey :PCERT_PUBLIC_KEY_INFO
                                ):DWORD ; stdcall;
//+-------------------------------------------------------------------------
...
var
  ...
  clCryptSetProvParam: TclCryptSetProvParam;
  ...
  clCertGetPublicKeyLength: TclCertGetPublicKeyLength;
  ...
  clCryptMsgGetParam: TclCryptMsgGetParam;
  clCryptMsgControl: TclCryptMsgControl;
  clCryptMsgUpdate: TclCryptMsgUpdate;
  clCryptMsgClose: TclCryptMsgClose;
  clCryptMsgOpenToDecode: TclCryptMsgOpenToDecode;
...
begin
  InitCrypt();
  if not Assigned(clCryptSetProvParam) then
    @clCryptSetProvParam := clGetProcAddress(ADVAPIModule, 'CryptSetProvParam', False);

  Result := clCryptSetProvParam(hProv, dwParam, pbData, dwFlags);
end;
...
begin
  InitCrypt();
  if not Assigned(clCertGetPublicKeyLength) then
    @clCertGetPublicKeyLength := clGetProcAddress(CryptModule, 'CertGetPublicKeyLength', False);

  Result := clCertGetPublicKeyLength(dwCertEncodingType, pPublicKey);
end;
...
begin
  InitCrypt();
  if not Assigned(clCryptMsgGetParam) then
    @clCryptMsgGetParam := clGetProcAddress(CryptModule, 'CryptMsgGetParam', False);

  Result := clCryptMsgGetParam(hCryptMsg, dwParamType, dwIndex, pvData, pcbData);
end;
...
begin
  InitCrypt();
  if not Assigned(clCryptMsgControl) then
    @clCryptMsgControl := clGetProcAddress(CryptModule, 'CryptMsgControl', False);

  Result := clCryptMsgControl(hCryptMsg, dwFlags, dwCtrlType, pvCtrlPara);
end;
...
begin
  InitCrypt();
  if not Assigned(clCryptMsgUpdate) then
    @clCryptMsgUpdate := clGetProcAddress(CryptModule, 'CryptMsgUpdate', False);

  Result := clCryptMsgUpdate(hCryptMsg, pbData, cbData, fFinal);
end;

function CryptMsgClose(hCryptMsg :HCRYPTMSG):BOOL ; stdcall;
begin
  InitCrypt();
  if not Assigned(clCryptMsgClose) then
    @clCryptMsgClose := clGetProcAddress(CryptModule, 'CryptMsgClose', False);

  Result := clCryptMsgClose(hCryptMsg);
end;
...
begin
  InitCrypt();
  if not Assigned(clCryptMsgOpenToDecode) then
    @clCryptMsgOpenToDecode := clGetProcAddress(CryptModule, 'CryptMsgOpenToDecode', False);

  Result := clCryptMsgOpenToDecode(dwMsgEncodingType,
