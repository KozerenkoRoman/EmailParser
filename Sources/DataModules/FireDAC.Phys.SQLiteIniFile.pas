...
type
  ...
  protected
    ...
    function ReadStrings(const ASection, AKey: String; AStrings: TStrings; const AAppend: Boolean = False): Integer;
    ...
    procedure ReadSection(const ASection: String; AStrings: TStrings; const AAppend: Boolean = False);
    procedure ReadSections(const AStrings: TStrings; const ASection: string = ''; const AAppend: Boolean = False);
    procedure ReadSectionValues(const ASection: String; AStrings: TStrings; const AAppend: Boolean = False);
    procedure ReadStream(const ASection, AKey: String; AStream: TStream);
    procedure WriteStream(const ASection, AKey: String; AStream: TStream; const ACompress: TStreamCompressionLevel);
  ...
  public
    ...
    constructor Create(const AFileName: String; const APassword: String; const AStreamCompressionLevel: TStreamCompressionLevel); overload;
    destructor Destroy; override;
    ...
    procedure Export(const AIniFile: TCustomIniFile; ASection: String = ''; AExcludeSectionPrefix: Boolean = True); overload;
    procedure Export(const AIniFileName: String; ASection: String = ''; AExcludeSectionPrefix: Boolean = True); overload;
    ...
    property StreamCompressionLevel: TStreamCompressionLevel read FStreamCompressionLevel write FStreamCompressionLevel;
  end;
...
const
  SQL_CreateTables = 'create table if not exists SECTIONS('                                             + sLineBreak +
                     '  SECTION_ID integer primary key,'                                                + sLineBreak +
                     '  SECTION_NAME text);'                                                            + sLineBreak +
...
begin
  ...
  try
    ExecSQL('delete from KEYS;' + 'delete from SECTIONS;');
    if lStartTransaction then
      Commit;
  except
    if lStartTransaction then
      RollBack;
...
begin
  ...
  begin
    Prepare(ASQL);
    for I := 0 to ParamDefsCount - 1 do
      TSQLiteBind.Create(Params);
    if SameText(Copy(ASQL, 1, 6), 'select') then
      for I := 0 to ColumnDefsCount - 1 do
        TSQLiteColumn.Create(Columns);
  end;
end;
...
function TFDSQLiteIniFileDB.GetKeyID(const ASectionID: Int64; const AKey: String): Int64;
begin
  Result := -1;
...
procedure TFDSQLiteIniFileDB.ReadSection(const ASection: String; AStrings: TStrings; const AAppend: Boolean);
begin
  if not AAppend then
    AStrings.Clear;
  ...
  try
    with CreateStatement(SQL_SelectSection) do
      try
        ...
        Execute;
        while Fetch do
          AStrings.Add(Columns[0].AsString);
      finally
        Free;
      end;
  finally
...
procedure TFDSQLiteIniFileDB.ReadSections(const AStrings: TStrings; const ASection: string; const AAppend: Boolean);
begin
  if not AAppend then
    AStrings.Clear;
  AStrings.BeginUpdate;
...
procedure TFDSQLiteIniFileDB.ReadSectionValues(const ASection: String; AStrings: TStrings; const AAppend: Boolean);
begin
  if not AAppend then
    AStrings.Clear;
  AStrings.BeginUpdate;
...
procedure TFDSQLiteIniFileDB.ReadStream(const ASection, AKey: String; AStream: TStream);
var
  B: Pointer;
  L: Integer;
begin
...
function TFDSQLiteIniFileDB.ReadString(const ASection, AKey: String; const ADefault: String): String;
begin
  Result := ADefault;
  with CreateStatement(SQL_SelectKeyValue) do
    try
      ...
      Execute;
      if Fetch then
        Result := Columns[0].AsString;
    finally
      Free;
    end;
end;

function TFDSQLiteIniFileDB.ReadStrings(const ASection, AKey: String; AStrings: TStrings;
...
procedure TFDSQLiteIniFileDB.WriteStream(const ASection, AKey: String; AStream: TStream;
...
begin
  ...
  try
    ...
    if iSectionID < 0 then
      iSectionID := InsertSection(ASection);
...
procedure TFDSQLiteIniFileDB.WriteString(const ASection, AKey, AValue: String);
var
  iKeyID: Int64;
  iSectionID: Int64;
begin
...
procedure TFDSQLiteIniFileDB.WriteStrings(const ASection, AKey: String; AStrings: TStrings);
var
  lStartTransaction: Boolean;
  I: Integer;
begin
...
constructor TFDSQLiteIniFile.Create(const AFileName: String; const APassword: String; const AStreamCompressionLevel: TStreamCompressionLevel);
begin
...
procedure TFDSQLiteIniFile.Export(const AIniFileName: String; ASection: String; AExcludeSectionPrefix: Boolean);
var
...
procedure TFDSQLiteIniFile.Export(const AIniFile: TCustomIniFile; ASection: String; AExcludeSectionPrefix: Boolean);
var
  oKeys: TStringList;
...
procedure TFDSQLiteIniFile.Import(const AIniFileName: String; ASectionPrefix: String);
var
  oIniFile: TIniFile;
begin
...
procedure TFDSQLiteIniFile.LoadFromFile(const ASection, AKey, AFileName: String);
var
  oStream: TMemoryStream;
begin
  oStream := TMemoryStream.Create;
...
procedure TFDSQLiteIniFile.Import(const AIniFile: TCustomIniFile; ASectionPrefix: String);
var
  lStartTransaction: Boolean;
...
function TFDSQLiteIniFile.ReadDate(const ASection, AKey: string; ADefault: TDateTime): TDateTime;
var
  S: String;
begin
  Result := ADefault;
...
function TFDSQLiteIniFile.ReadDateTime(const ASection, AKey: string; ADefault: TDateTime): TDateTime;
var
  S: String;
begin
  Result := ADefault;
...
function TFDSQLiteIniFile.ReadFloat(const ASection, AKey: string; ADefault: Double): Double;
var
  S: string;
begin
  Result := ADefault;
...
procedure TFDSQLiteIniFile.ReadSection(const ASection: string; AStrings: TStrings);
begin
...
procedure TFDSQLiteIniFile.ReadSections(const ASection: String; AStrings: TStrings);
begin
  FDatabase.ReadSections(AStrings, ASection + '%');
end;

procedure TFDSQLiteIniFile.ReadSectionValues(const ASection: string; AStrings: TStrings);
begin
  FDatabase.ReadSectionValues(ASection, AStrings);
end;

procedure TFDSQLiteIniFile.ReadStream(const ASection, AKey: String; AStream: TStream);
begin
  FDatabase.ReadStream(ASection, AKey, AStream);
end;

function TFDSQLiteIniFile.ReadString(const ASection, AKey, ADefault: String): String;
begin
  Result := FDatabase.ReadString(ASection, AKey, ADefault);
end;

function TFDSQLiteIniFile.ReadStrings(const ASection, AKey: string; AStrings: TStrings; const AAppend: Boolean)
  : Integer;
begin
  Result := FDatabase.ReadStrings(ASection, AKey, AStrings, AAppend);
end;

function TFDSQLiteIniFile.ReadTime(const ASection, AKey: string; ADefault: TDateTime): TDateTime;
var
  S: String;
begin
  Result := ADefault;
...
procedure TFDSQLiteIniFile.WriteDate(const ASection, AKey: string; AValue: TDateTime);
begin
  WriteString(ASection, AKey, DateToStr(AValue, FFormatSettings));
end;

procedure TFDSQLiteIniFile.WriteDateTime(const ASection, AKey: string; AValue: TDateTime);
begin
  WriteString(ASection, AKey, DateTimeToStr(AValue, FFormatSettings));
end;

procedure TFDSQLiteIniFile.WriteFloat(const ASection, AKey: string; AValue: Double);
begin
  WriteString(ASection, AKey, FloatToStr(AValue, FFormatSettings));
end;

procedure TFDSQLiteIniFile.WriteStream(const ASection, AKey: String; AStream: TStream);
begin
...
procedure TFDSQLiteIniFile.WriteStrings(const ASection, AKey: string; const AValue: TStrings);
begin
  FDatabase.WriteStrings(ASection, AKey, AValue);
end;

procedure TFDSQLiteIniFile.WriteTime(const ASection, AKey: string; AValue: TDateTime);
begin
